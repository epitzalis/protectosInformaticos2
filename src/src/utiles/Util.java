...
public class Util {
	...
	public static int dados() {
		...
		if(dado1 == dado2) {
			...
			int dado4 =  (int)(Math.random()*7 + 1);
			System.out.println("Has sacado 2 dobles, dados: 3) "+dado2+" 4) "+dado4);
			if(dado3 == dado4) {
				int suma2 = suma + dado3 + dado4;
				int dado5 = (int)(Math.random()*7 + 1);
				int dado6 = (int)(Math.random()*7 + 1);
				System.out.println("Has sacado 3 dobles, vas a la carcel, dados: 5) "+dado5+" 6) "+dado6);
				if(dado5 == dado6) {
					resultado= 0;
				}
				else {
					resultado = suma2 + dado5 + dado6; 
				}
				
			}else {
				resultado = suma + dado3 +dado4;
			}
		}else {
