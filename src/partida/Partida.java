	...
	public static void disminuirDineroJugador(String nombreJugador, int cantidad) {
		for (int i = 0; i < jugadores.length; i++) {
			if (jugadores[i].getNombre().equals(nombreJugador)) {
				...
				jugadores[i].setDinero(totalDinero);
				System.out
						.println("El dinero de " + jugadores[i].getNombre() + " ha disminuido a " + totalDinero + "€");
			}
		}
	}
	...
	public void comenzar() {
		...
		while (isFinalizada == false) {
			...
			if (jugadorActual.getDinero() > 0) { // Solo mostrarmos el turno de los jugadores que tienen dinero
				System.out.println("\nEs el turno de " + jugadorActual.getNombre() + " (Dispones de "
						+ jugadorActual.getDinero() + "€), elija una opcion:");

				if (jugadorActual.isCarcel() == true && jugadorActual.getContador() < 3) { // Miramos si el jugador esta
																							// en la carcel encerrado
					...
					case 2: // Guardar Partida
						Fichero.exportarFichero(this.jugadores, this.cajaBanca, this.nombreJugadorActual,
								Tablero.getPropiedades());
						break;
					case 3:
